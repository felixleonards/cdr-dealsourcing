...
import feedparser
from scraper.sources.carbon_herald import get_articles, FEED_URL

RSS_FIXTURE = """<?xml version="1.0"?>
<rss version="2.0">
  <channel>
    <item><title>Test</title><link>https://carbon-herald.com/article-1</link></item>
  </channel>
</rss>"""
...
def test_get_articles_uses_correct_feed():
    with patch('scraper.sources.rss.feedparser.parse') as mock_parse:
        ...
        resp_lib.add(resp_lib.GET, "https://carbon-herald.com/article-1", body="<html><body>text</body></html>")

        articles = get_articles()

        assert len(articles) == 1
        assert articles[0].url == "https://carbon-herald.com/article-1"
